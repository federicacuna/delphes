...
TMatrixDSym TrkUtil::RegInv(TMatrixDSym &Min)
{
  ...
  {
    if(M(i, i) != 0.0)
      D(i, i) = 1. / TMath::Sqrt(TMath::Abs(M(i, i)));
    else
      D(i, i) = 1.0;
  }
  R = M.Similarity(D);
...
Double_t TrkUtil::Nclusters(Double_t begam, Int_t Opt)
{
  ...
  {
  case 0:
    std::copy(ncl_He_Iso, ncl_He_Iso + Npt, ncl); // He-Isobutane
    break;
  case 1:
    std::copy(ncl_He, ncl_He + Npt, ncl); // pure He
    break;
  case 2:
    std::copy(ncl_Ar_Eth, ncl_Ar_Eth + Npt, ncl); // Argon - Ethane
    break;
  case 3:
    std::copy(ncl_Ar, ncl_Ar + Npt, ncl); // pure Argon
    break;
  }
  //
  Double_t interp = 0.0;
...
// Return the enerdy loss value
Bool_t TrkUtil::IonEnergyLoss(Double_t &Eloss, Double_t mass, TVectorD Par, Double_t &dedx)
{
  //
  // Units are meters/Tesla/GeV
  //
  ...
  Double_t fTruncatedMeanFraction = 0.7;
  ...
  {
    // No geometry set so send a warning and return 0
    std::cout << "TrkUtil::IonEnergyLoss() called without a volume defined" << std::endl;
  }
  else
    tLen = TrkLen(Par);

  //******************************************************************
  // Now get the energy loss value                      ****
  //******************************************************************
  //
  Double_t muEloss = 0.0; // mean number of Eloss
  Double_t sgmEloss = 0.0; //sgm eloss
  Double_t bg = 0.0; // beta*gamma
  Eloss = 0.0;
  Double_t tmpEloss = 0.0;
  if(tLen > 0.0)
  {
    ...
    else
    {
      ...
      // TString data = "/afs/cern.ch/user/f/fcuna/Delphes/dchgeometry.txt";
      // fG->ReadDCH(data);
      // fG.SetGeom(/afs/cern.ch/user/f/fcuna/Delphes/dchgeometry.txt);
      TVector3 p = ParToP(Par);
      TVector3 x = ParToX(Par);
      SolTrack TU2(x, p, getSolGeom());
      bg = p.Mag() / mass;
      // std::cout << " bg " << bg << std::endl;
      d = (tLen / TU2.nmHit()) * 1e2;
      // std::cout<<"number of layers "<<fG.Nl()<< "number of hit  "<<TU2.nmHit()<<" d "<<d<<std::endl;
      // muEloss = coeffa_mpv(bg) * d + coeffb_mpv(bg);// EnergyLoss(bg) * d; // Avg.Eloss//
      ...
      Double_t phi;
      Double_t zz;
      // for(int i=0;i<fG.Nl();++i){
      //   if(TU2.HitLayer(i,R,phi,zz)){
      ...
      for(Int_t j = 0; j < /*TMath::Nint(tLen*1e2) */ TU2.nmHit(); ++j)
      {
        ...
        // std::cout<<"mueloss per hit "<<gRandom->Landau(muEloss,sgmEloss)<<std::endl;
      }

      std::sort(elosses.begin(), elosses.end());
      int cut = TMath::Nint(fTruncatedMeanFraction * elosses.size());
      Double_t sumVec = 0.0;
      Double_t meanVec = 0.0;
      for(int i = 0; i < cut; ++i)
      {
        sumVec += elosses[i];
        meanVec = sumVec / ((float)cut);
      }

      // eloss_truncmean = TruncatedMean(elosses, fTruncatedMeanFraction);
      dedx = meanVec;
      // std::cout<< " mass "<< mass<< " Eloss "<<Eloss<<" dEdx "<<dedx<<"elosses size "<<elosses.size()<<std::endl;
    }
  }
  //
  return Signal;
...
void TrkUtil::setGeomfile(TString Geomdata)
{
  fG.ReadDCH(Geomdata.Data());
}

SolGeom *TrkUtil::getSolGeom()
{
  return &fG;
}
